 ...
 * @LastEditors: Chengya
 * @LastEditTime: 2024-02-02 15:25:49
-->
<template>
  <div id="app">
...
export default {
  ...
  methods: {
    ...
    unbindEvent() {
      //this.$bus.$off('changeListItem')
      //this.$bus.$off('delListItem')
      this.$bus.$off(['delListItem', 'changeListItem'])
    },
    //编辑时更新 todoLIST
    updateTodoList(newItem) {
      ...
      console.log(this.todoList, 'todoList======95')
    },
  },
  mounted() {
    this.$bus.$on('delListItem', this.delListItem)
    this.$bus.$on('changeListItem', this.changeListItem)
    this.$bus.$on('updateTodoList', this.updateTodoList)
  },
  beforeDestroy() {
    this.unbindEvent()
  },
